...
const BookShow = () => {
  ...
  /* 
  When a user clicks "Pay Now", StripeCheckout calls the token callback function (onToken) we provided. 
  This function (onToken) receives a token object from Stripe containing payment details, which we will
  then use to process the payment on your server. 
  we would send this token to our server (via an API call) along with other relevant information (like the total amount
  and selected items) to process the payment securely using Stripe’s API.
  */
	...
	return (
		<>
			{show && (
                ...
                // generate a payment component for card details
								<StripeCheckout
									token={onToken}
									amount={selectedSeats.length * show.ticketPrice * 100}
									stripeKey="pk_test_51QPSVmCHZ8mVDYIhB0w2HJM9e0GvE9WPUXs0akIgBIUll26QckzW4XsQhh8mnUz9RQCq2XwZ4zrd6R5oRCmL2ddz00bM6cA61J"
								>
